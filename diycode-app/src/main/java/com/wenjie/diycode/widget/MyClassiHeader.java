 ...
 * @FileName: com.smartrefreshlayout.A.java
 * @Author: wenjie
 * @Date: 2017-07-21 13:51
 * @Description:
 * @Version:
 */
public class MyClassiHeader extends RelativeLayout implements RefreshHeader {
    ...
    AnimationDrawable animationDrawable;//自定义加载进度样式
    private DateFormat mFormat = new SimpleDateFormat("上次更新时间 yyyy-M-d HH:mm", Locale.CHINA);
    private SpinnerStyle mSpinnerStyle = SpinnerStyle.Translate;
    private SharedPreferences mShared;

    //<editor-fold desc="RelativeLayout">
    public MyClassiHeader(Context context) {
        super(context);
        this.initView(context, null, 0);
    }

    public MyClassiHeader(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.initView(context, attrs, 0);
    }

    public MyClassiHeader(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        mProgressView.setImageDrawable(ContextCompat.getDrawable(context , R.drawable.dialog_loading));//自定义加载进度样式
        mProgressView.setImageDrawable(mProgressDrawable);
//        animationDrawable = (AnimationDrawable) mProgressView.getDrawable();//自定义加载进度样式
...
//        animationDrawable.start();//自定义加载进度样式
        mProgressDrawable.start();
    }

    @Override
    public int onFinish(RefreshLayout layout, boolean success) {
//        animationDrawable.stop();//自定义加载进度样式
    ...
    public void onStateChanged(RefreshLayout refreshLayout, RefreshState oldState, RefreshState newState) {
        switch (newState) {
            ...
            case Refreshing:
                mHeaderText.setText(REFRESH_HEADER_REFRESHING);
//                mProgressView.setImageDrawable(animationDrawable);//自定义加载进度样式
                mProgressView.setImageDrawable(mProgressDrawable);
                mProgressView.setVisibility(VISIBLE);
                mArrowView.setVisibility(GONE);
                break;
    ...
    public MyClassiHeader setLastUpdateTime(Date time) {
        mLastTime = time;
    ...
    public MyClassiHeader setTimeFormat(DateFormat format) {
        mFormat = format;
        mLastUpdateText.setText(mFormat.format(mLastTime));
        return this;
    }

    public MyClassiHeader setSpinnerStyle(SpinnerStyle style) {
        this.mSpinnerStyle = style;
        return this;
    }

    public MyClassiHeader setAccentColor(int accentColor) {
